...
sim_out = 
  ...
  mutate(
    ...
    seven_day_av_g=cut(seven_day_average, breaks = c(0,10,30,100,200,450), include.lowest = TRUE),
...
# p1 <- plot_specificR_without_text(roc = roc1, R0spec = 1) + ggtitle("1-day absolute increase, R=1")
# p1
...
#plot_specificR_without_text(roc = roc2, R0spec = 1) + ggtitle("1-day relative increase, R=1")
...
## Combine with second simulation using larger seeds

sim_data2 = readRDS("../Data/sim_data2b.rds")

## Data management
sim_out2 = 
  do.call("rbind",sim_data2) %>%
  tbl_df() %>%
  gather("day","cases",5:64) %>%
  mutate(day=as.numeric(gsub("X","",day))) %>%
  mutate(combi_id=paste0(n,"-",sim)) %>%
...
sim_tot <- rbind(sim_orig, sim_out2) %>%
  filter(seed %in% c(10,50,90,150,200,250), 
         day <= 30, day > 15)

trajectories_tot <- 
  ggplot(sim_tot) +
  geom_line(aes(x=day,y=cases,group=combi_id,colour=R0),size=0.1,alpha=.1) +
  facet_grid(seed~p,scales="free") +
  labs(x="Day",y="Cases")

trajectories_tot

roc1tot = get_roc(data=sim_tot,response="R0",predictor="delta_abs_cases")

plot_allR(roc = roc1tot) + ggtitle("1-day absolute increase")
p1 <- plot_specificR_without_text(roc=roc1tot, R0spec=1) + 
  ggtitle("1-day absolute increase") +
  geom_text(aes(x=1-sp, y=se, label=thresholds, 
                colour=ifelse(youdan==youdan_cutoff, "2", 
                              ifelse(dist==distance_cutoff, "3",NA)))) 
  
p1

roc10tot = get_roc(data=sim_tot,response="R0",predictor="numb_increases_abs10")

plot_allR(roc = roc10tot) + ggtitle("Number of day-to-day increases within last 10 days")
p2 <- plot_specificR(roc=roc10tot, R0spec=1) + 
  ggtitle("1-day absolute increase") 

p2

roc3tot = get_roc(data=sim_tot,response="R0",predictor="numb_greater_abs10")

plot_allR(roc = roc3tot) + ggtitle("Number of times reported cases are lower in last 10 days")
p3 <- plot_specificR(roc=roc3tot, R0spec=1) 
  + ggtitle("Number of times reported cases are lower in last 10 days")

p3


roc11_tot = get_roc(data=sim_tot,response="R0", predictor="numb_3day_av_greater_abs10")

plot_allR(roc = roc11_tot) + ggtitle("3-day average greater than how many cases of prev. 10 days")
p4 <- plot_specificR(roc=roc11_tot, R0spec=1) + ggtitle("3-day average greater than how many cases of prev. 10 days")

p4


# save(p1, p4, roc11_tot, roc1tot, trajectories_tot, file="../Data/report_roc_plots.rda")
